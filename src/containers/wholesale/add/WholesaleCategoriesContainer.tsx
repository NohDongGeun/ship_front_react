...
const WholesaleCategoriesContainer: React.FC = () => {
    ...
    const [selectedMiddleCategories, setSelectedMiddleCategories] = useState<
        IMiddleCategory[]
    >([]);
    const mainCategories = useAddWholesaleStore(
        (state) => state.wholesale.mainCategories
    );
    const setter = useAddWholesaleStore((state) => state.update);
    const selectCategory = useAddWholesaleStore(
        (state) => state.selectCategory
    );
    const deleteCategory = useAddWholesaleStore(
        (state) => state.deleteCategory
    );
    const setterMiddleCategories = useAddWholesaleStore(
        (state) => state.setMiddleCategories
    );
    const selectMiddleCategory = useAddWholesaleStore(
...
export default React.memo(WholesaleCategoriesContainer);
