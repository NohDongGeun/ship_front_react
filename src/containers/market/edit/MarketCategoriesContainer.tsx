...
const MarketCategoriesContainer: React.FC = () => {
    ...
    const categories = useEditMarketStore((state) => state.market.categories); // 현재 스토어에 선택되어 있는 카테고리 목록
    ...
    const updateMarket = useEditMarketStore((state) => state.updateMarket);
        ...
        (
            ...
            if (storeCategoryIndex === -1) {
                ...
                if (storeMiddleCategoryIndex === -1) {
                    ...
                    updateMarket({ categories: tempStoreCategories });
                } else {
    ...
    const onClickDeleteCategory = useCallback(
        (index: number) => {
            ...
            updateMarket({ categories: tempCategories });
        },
        [categories]
    );
